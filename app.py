...
# to improve: change competitors df on % change & name of competitors, improve table showing (b/s, i/s, cf/s)
''' new features: 
1. cross sectional multiple stock comparison on valuation, ratio, trading info
2. trading page
    a. scrape basic info about volumne bid/ask basic ratio
    b. deprecate sqlite3 and create postgres cluster to manage databases in >= 2 machines (will need to pay money)
    c. trading page have big form to fill in, with limitations (e.g. can't sell > inventory, can't buy > capital)
3. b/s i/s cf/s add changes comparison to last data (become more cross sectional)
(4. add other markets geh stocks)
'''
#
...
def run_result(stocks, request_data):
    ...
    data_id_name = [item.replace(" ", "_").lower() for item in request_data]
